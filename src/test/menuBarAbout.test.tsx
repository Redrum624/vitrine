...
const APP_INFO = {
  name: 'Photo Editor Pro', version: '1.11.0', description: 'Professional photo editor',
...
describe('MenuBar — Help ("?") menu / About', () => {
  it('opens an About dialog showing the version and runtime info', async () => {
    ...
    fireEvent.click(screen.getByText('About Photo Editor Pro'));
    const dlg = await screen.findByRole('dialog', { name: 'About Photo Editor Pro' });
  ...
  it('opens the repository link via openExternalUrl', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Help' }));
    fireEvent.click(screen.getByText('About Photo Editor Pro'));
    await screen.findByRole('dialog', { name: 'About Photo Editor Pro' });
